...
| `bastion forwards list` | Show the `[forwards]` ingress DNAT/port-forward rules (edge). |
| `bastion forwards add <name> <proto/port> <dest_ip:port> [--via IFACE] [--snat IP] [--dry-run] [--yes]` | Add/replace a port-forward: DNAT a WAN `<proto>/<port>` to an internal/mesh `<dest_ip>:<port>`. `--via` scopes the forwarded flow's egress interface; `--snat IP` adds a fixed source-NAT (the box's own address on `--via`, needed for a mesh/overlay egress so the return path holds; `--snat` requires `--via`). Previews the exact `+`/`-` nftables rule, then generate + reload; same `--dry-run`/`--yes`/clobber-gate behavior as `zones`. Edge-only. |
| `bastion forwards remove <name> [--dry-run] [--yes]` | Delete a port-forward: preview the removed rule, then generate + reload. |
